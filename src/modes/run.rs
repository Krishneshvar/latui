...
impl RunMode {
    ...
    fn rebuild_searchable_history(&mut self) {
        self.searchable_history = self.history
            .iter()
            .map(|entry| {
                let item = Item {
                ...
                SearchableItem::new(item)
                    .with_field("command", &entry.command, 10.0)
            })
            .collect();
    }
    ...
    fn get_recent_commands(&self) -> Vec<Item> {
        let mut results: Vec<(Item, f64)> = self.history
            .iter()
...
impl Mode for RunMode {
    ...
    fn load(&mut self) -> Result<(), LatuiError> {
        ...
        tracing::info!("Run mode loaded with {} commands in history", self.history.len());
        Ok(())
    }
    ...
    fn search(&mut self, query: &str) -> Vec<Item> {
        ...
        if !self.searchable_history.is_empty() {
            let mut history_results = self.search_engine.search_scored(q, &self.searchable_history);
            ...
            if let Some(ref tracker) = self.frequency_tracker {
                ...
                history_results.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
            }
